...
describe('NotificationListener', () => {
  ...
  describe('POST_REPLY 이벤트', () => {
    it('게시글 답글 알림을 생성한다', async () => {
      const [actor, targetUser] = await prisma.user.createManyAndReturn({
        data: [
          {
            provider: 'kakao',
      ...
      // when
      await notificationListener.handlePostReply({
        postId: post.id,
        actorId: actor.id,
    ...
    it('부모댓글 작성자와 대댓글 작성자가 같을 경우 알림데이터를 생성하지 않는다', async () => {
      const [actor, targetUser] = await prisma.user.createManyAndReturn({
        data: [
          {
            provider: 'kakao',
    ...
    it('부모 댓글이 isDeleted 상태일 경우 알림데이터를 생성하지 않는다', async () => {
      const [actor, targetUser] = await prisma.user.createManyAndReturn({
        data: [
          {
            provider: 'kakao',
  ...
  describe('POST_MENTION 이벤트', () => {
    it('게시글 멘션 알림을 생성한다', async () => {
      const [actor, targetUser] = await prisma.user.createManyAndReturn({
        data: [
          {
            provider: 'kakao',
      ...
      // when
      await notificationListener.handlePostMention({
        postId: post.id,
        actorId: actor.id,
        mentionedUserId: targetUser.id,
      });
    ...
    it('구독하고 있지 않으면 알림 데이터를 생성하지 않는다', async () => {
      const [actor, targetUser] = await prisma.user.createManyAndReturn({
        data: [
          {
            provider: 'kakao',
