...
describe('PATCH /commission-works/:id/cancel - 커미션 신청 취소', () => {
  ...
  it('accessToken이 없을 때 401을 반환한다', async () => {
    ...
    const { status } = await request(app.getHttpServer()).patch(
      `/commission-works/${work.id}/cancel`,
    );
  ...
  it('workId가 UUID 형식이 아니면 400을 반환한다', async () => {
    ...
    const { status } = await request(app.getHttpServer())
      .patch('/commission-works/not-a-uuid/cancel')
  ...
  it('존재하지 않는 workId면 404를 반환한다', async () => {
    ...
    const { status, body } = await request(app.getHttpServer())
      .patch('/commission-works/00000000-0000-0000-0000-000000000000/cancel')
      .set('Authorization', `Bearer ${accessToken}`);
  ...
  it('신청자(의뢰인) 본인이 아니면 403을 반환한다', async () => {
    const { user: author, accessToken } = await createAuthor();
    const { user: client } = await createClient();
    const work = await createWork(author.id, client.id);

    const { status, body } = await request(app.getHttpServer())
      .patch(`/commission-works/${work.id}/cancel`)
      .set('Authorization', `Bearer ${accessToken}`);
  ...
  it('정상 취소 시 200과 id를 반환하고 DB에 CANCELED 상태가 저장된다', async () => {
    const { user: author } = await createAuthor();
    const { user: client, accessToken } = await createClient();
    const work = await createWork(author.id, client.id);
    ...
    const events = await prisma.commissionWorkEvent.findMany({
      where: { workId: work.id, type: 'CANCELED' },
    });
    expect(events).toHaveLength(1);
  });

  describe('채팅방 / 시스템 메시지', () => {
    it('취소 시 신청자가 작가에게 COMMISSION_CANCELED 시스템 메시지를 보낸다', async () => {
      const { user: author } = await createAuthor();
      const { user: client, accessToken } = await createClient();
      const work = await createWork(author.id, client.id);
      ...
      const messages = await prisma.chatMessage.findMany({
        where: { type: 'COMMISSION_CANCELED', referenceId: work.id },
      });
