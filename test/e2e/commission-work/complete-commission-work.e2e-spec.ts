...
describe('PATCH /commission-works/:id/complete - 커미션 작업 완료', () => {
  ...
  async function createWork(
    authorId: string,
    clientId: string,
    status: CommissionWorkStatus = 'FINAL',
  ) {
  ...
  it('accessToken이 없을 때 401을 반환한다', async () => {
    ...
    const { status } = await request(app.getHttpServer()).patch(
      `/commission-works/${work.id}/complete`,
    );
  ...
  it('workId가 UUID 형식이 아니면 400을 반환한다', async () => {
    ...
    const { status } = await request(app.getHttpServer())
      .patch('/commission-works/not-a-uuid/complete')
  ...
  it('존재하지 않는 workId면 404를 반환한다', async () => {
    ...
    const { status, body } = await request(app.getHttpServer())
      .patch('/commission-works/00000000-0000-0000-0000-000000000000/complete')
      .set('Authorization', `Bearer ${accessToken}`);
  ...
  it('신청자 본인이 아니면 403을 반환한다', async () => {
    ...
    expect(body.errorCode).toBe('NOT_COMMISSION_CLIENT');
  });

  it('PENDING 상태면 409를 반환한다', async () => {
    const { user: author } = await createAuthor();
    const { user: client, accessToken } = await createClient();
    const work = await createWork(author.id, client.id, 'PENDING');
    ...
    expect(body.errorCode).toBe('WORK_NOT_COMPLETABLE');
  });

  it('FINAL 상태에서 신청자가 완료 시 200과 id를 반환하고 DB에 COMPLETED 상태가 저장된다', async () => {
    const { user: author } = await createAuthor();
    const { user: client, accessToken } = await createClient();
    const work = await createWork(author.id, client.id, 'FINAL');
  ...
  it('ACCEPTED 상태에서도 신청자가 완료할 수 있다', async () => {
    const { user: author } = await createAuthor();
    const { user: client, accessToken } = await createClient();
